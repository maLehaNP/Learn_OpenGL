...
int main() {
	...
	// Projection matrix
	projection = glm::perspective(glm::radians(60.0f), static_cast<float>(WINDOW_WIDTH) / static_cast<float>(WINDOW_HEIGHT), 0.1f, 100.0f);
	//projection = glm::ortho(0.0f, static_cast<float>(WINDOW_WIDTH), 0.0f, static_cast<float>(WINDOW_HEIGHT), 0.1f, 100.0f);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));*/
		/*view = glm::mat4(1.0f);
		view = glm::translate(view, glm::vec3(-x_mean, -y_mean, -z_mean));
...
void framebuffer_size_callback(GLFWwindow* window, int widht, int height) {
	...
	projection = glm::perspective(glm::radians(45.0f), static_cast<float>(widht) / static_cast<float>(height), 0.1f, 100.0f);  // Updating uspect ratio
}
